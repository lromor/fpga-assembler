...
TEST(BigEndianSpanTest, Read32WithEmptySpan) {
  ...
  auto words = make_big_endian_span<uint32_t>(bytes);
...
TEST(BigEndianSpanTest, Read32WithExactBytes) {
  ...
  EXPECT_EQ(static_cast<uint32_t>(words[0]), static_cast<uint32_t>(0x00010203));
}
...
TEST(BigEndianSpanTest, Read32WithMultipleWords) {
  ...
  EXPECT_EQ(static_cast<uint32_t>(words[0]), static_cast<uint32_t>(0x00010203));
  EXPECT_EQ(static_cast<uint32_t>(words[1]), static_cast<uint32_t>(0x04050607));
}
...
TEST(BigEndianSpanTest, Write32) {
  ...
  const std::vector<uint8_t> expected{0x4, 0x5, 0x6, 0x7};
